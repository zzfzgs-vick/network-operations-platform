...
- `apps/platform/src/http/contract-exception.filter.ts`
- `apps/platform/src/modules/platform-health/platform-health.module.ts`
- `scripts/db-test.mjs`
- `tests/integration/audit/run-security.mjs`
- `tests/integration/auth/local-auth.test.mjs`

范围修正理由：T012 的默认拒绝必须接入真实 API 全局 Guard，授权指标必须进入既有低基数运行指标，`rbac` 与 `authorization` 必须接入统一测试入口；事务内的权威授权复核拒绝必须映射到既有 403 错误 envelope；新增迁移后，T011 固定最新迁移版本的回归断言必须改为基于迁移权威清单动态计算。以上变更不引入 Session、TOTP、后续业务 API 或新依赖。
