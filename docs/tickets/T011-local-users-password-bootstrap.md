...
- `docs/tickets/T011-local-users-password-bootstrap.md`
...
T011 的完成定义要求成熟 Argon2id 密码实现、可执行的首管理员初始化命令、真实 PostgreSQL local-auth 测试和 password-policy 安全测试；当前 Platform 清单、唯一 npm 锁文件及测试分发器不在允许范围，因此需要最小范围修正。

T011 新增 0005 迁移后，T010 审计集成测试中固定的 v3→v4、固定 appliedCount 和空库迁移数量断言不再成立。测试应从仓库迁移权威清单动态计算最新版本、升级迁移数量和空库总迁移数量，同时保留重复执行、兼容性、pending 和审计行为断言。
